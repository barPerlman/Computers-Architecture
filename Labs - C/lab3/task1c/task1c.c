...
#include <string.h>
#include <stdlib.h>
#define MAX_SIZE 10000  //size for buffer

char buffer[MAX_SIZE];      //init buffer
char *arg=NULL;             //get the name of the infected file as cmd argument
...
typedef struct link link;   //create shortcut name for the link structure
...
void list_print(link *virus_list){
    link* p=virus_list; //init the head of list
    while(p!=NULL){
        printf("Virus name: %s\n",p->vir->virusName);
        printf("Virus size: %d\n",p->vir->SigSize);
        printf("signature:\n");
...
link* list_appendLast(link* virus_list,virus* data){
    ...
    if(virus_list==NULL)
        return newLink;
    //move the p pointer to point on thehar input[128] last link
...
void quit(){
    list_free(viruses);     //free the memory allocated to the list before exiting
    exit(0);
}
...
//get and read viruses file
    ...
    unsigned short endIndicator;                             //this get the size of the current virus or 0 if eof or error occurred
    virus *v=NULL;
    for (;fread(&endIndicator,sizeof(unsigned short),1, inputFile) != 0;) {
        ...
        v = (virus*)calloc(1,sizeOfVirus);
        if(v==NULL){
            fprintf(stderr,"couldn't allocate memory for virus struct.exiting...");
            exit(-1);
        }
...
struct fun_desc menu[] = { {"Load signatures",loadSigs}, {"Print signatures",printList},{"Detect viruses",detect_viruses},{"Quit",quit},{ NULL, NULL } };
