...
void RelocationTables() {
    ...
    int i, count, size, entsize, j, index;
    Elf32_Shdr *shdr = (Elf32_Shdr *)(map_start + header->e_shoff);
    char* stringHeaderTable = map_start + (shdr +  header->e_shstrndx)->sh_offset;
    Elf32_Sym * dyntab;
    char* stringTable;
    for (i = 0; i < header->e_shnum; i++) {
        char *section_name = stringHeaderTable + shdr[i].sh_name;
        if (shdr[i].sh_type == SHT_DYNSYM) {
            dyntab = (Elf32_Sym *)((char *)map_start + shdr[i].sh_offset);
        }
        else if (strcmp(section_name, ".dynstr") == 0) {
            stringTable = map_start + shdr[i].sh_offset;
        }
    ...
    for (i = 0; i < header->e_shnum; i++) {
        if (shdr[i].sh_type == SHT_REL) {
            Elf32_Rel * rel = (Elf32_Rel *)((char *)map_start + shdr[i].sh_offset);
            size = shdr[i].sh_size;
            entsize = shdr[i].sh_entsize;
            count = size/ entsize;
            fprintf(stdout, "Table name: %s\n", stringHeaderTable + shdr[i].sh_name);
            fprintf(stdout, "%-20s %-20s %-20s %-20s %-20s\n", "Offset","Info","Type","Sym.Value","Sym.Name");
            for (j = 0; j < count; j++) {
                index = ELF32_R_SYM(rel[j].r_info);
                fprintf(stdout, "%-20x %-20x %-20x %-20x %-20s\n", rel[j].r_offset, rel[j].r_info, ELF32_R_TYPE(rel[j].r_info),
                        dyntab[index].st_value,  stringTable + dyntab[index].st_name);
            }
        }
    }
}
