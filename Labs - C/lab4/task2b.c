...
void runDebugMode(char*,int,int);
void runRegularMode(char*,int,int);
...
int main (int argc , char* argv[], char* envp[])
{
	...
	if(isPrefix){
		  ...
		  if(!isDebugMode){
			  runRegularMode(p_pref,isPrefix,prefixSize);
		  }else{
			 runDebugMode(p_pref,isPrefix,prefixSize); 
		  }
...
void runDebugMode(char* p_pref,int isPrefix,int prefixSize){
	int res;
               ...
               for (bpos = 0; bpos < nread;) {	/*for each file print it's name*/
                  ...
                  if(isPrefix){ /*need to compare prefix*/
                   if(strncmp(p_pref,(d->d_name)-1,strlen(p_pref))==0){
						...
						system_call(SYS_WRITE,STDERR,(d->d_name)-1,strlen(d->d_name)+1); /*print the file name*/
	...
	/***************************************************************************************************************************/
	void runRegularMode(char* p_pref,int isPrefix,int prefixSize){
